...
export default function SignupPhonePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/otp?type=signup&method=phone&identifier=${encodeURIComponent(fullPhone)}`);
    } catch (error: any) {
      toast({
        variant: 'destructive',
        title: 'Error',
  ...
  return (
    ...
    >
      ...
      <form className="w-full max-w-sm space-y-5" onSubmit={handleSubmit}>
        ...
        {/* Phone Number */}
        <div>
          <label className="block text-body-sm font-body-sm text-on-surface font-medium px-1 mb-1">
            Phone Number
          </label>

          <div className="flex rounded-xl overflow-hidden border border-transparent bg-surface-container focus-within:bg-surface focus-within:border-secondary focus-within:ring-1 focus-within:ring-secondary transition-all">
            {/* Country selector */}
            ...
            <input
              ...
              required
              className="flex-1 px-4 py-3 text-sm text-gray-800 placeholder:text-gray-300 outline-none bg-transparent"
            />
          </div>
        </div>
        ...
        <button
          type="submit"
          disabled={isLoading || !firstName || !lastName || !phone || !agreed}
      ...
      <p className="flex flex-col mt-8 text-sm text-gray-500 text-center">
        ...
        <Link href="/login/phone" className="font-semibold text-blue-600 hover:underline">
          Sign in
        </Link>
      </p>
    </div>
