...
const SETTINGS_SECTIONS = [
  {
    title: 'Account',
    items: [
      { icon: 'person', label: 'Edit Profile', href: '/settings/profile' },
      { icon: 'phone', label: 'Phone Number', href: '/settings/security/login', value: '+234 808 ***' },
      { icon: 'email', label: 'Email Address', href: '/settings/security/login', value: 'a.kunle@email.com' },
  ...
  },
  {
    title: 'Preferences',
    items: [
      { icon: 'language', label: 'Country & Currency', href: '/settings/preferences', value: 'Nigeria - NGN' },
      { icon: 'notifications', label: 'Notifications', href: '/settings/notifications' },
      { icon: 'dark_mode', label: 'Appearance', href: '/settings/app', value: 'Light' },
    ],
  },
  {
    title: 'Seller',
    items: [
      { icon: 'store', label: 'My Listings', href: '/seller/inventory' },
      { icon: 'account_balance_wallet', label: 'Wallet & Payments', href: '/wallet/overview' },
      { icon: 'receipt_long', label: 'Sales History', href: '/orders/receipts' },
    ],
  },
  {
    title: 'Support',
    items: [
      { icon: 'help', label: 'Help Center', href: '/help/hub' },
      { icon: 'chat', label: 'Contact Support', href: '/help/safety-contact' },
      { icon: 'policy', label: 'Privacy Policy', href: '/legal/privacy' },
    ],
  },
];
  ...
  return (
    <div className="max-w-2xl mx-auto px-container-margin-mobile py-6 flex flex-col gap-6">
      <div className="flex items-center gap-4 p-5 bg-surface-container-low rounded-2xl">
        ...
        <Link href="/settings/profile" className="p-2 rounded-full hover:bg-surface-container transition-colors">
      ...
      <div className="glass-ai rounded-2xl p-5 flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="text-headline-lg-mobile font-headline-lg-mobile text-on-surface font-bold">NGN 54,200.00</div>
        </div>
        <Link href="/wallet/overview" className="bg-primary-container text-on-primary-container font-label-caps text-label-caps font-bold px-4 py-2 rounded-xl hover:bg-primary-fixed-dim transition-colors">
          Top Up
        </Link>
      </div>
      ...
      <Link href="/splash" className="block w-full text-center border border-error text-error font-body-sm text-body-sm font-semibold py-3 rounded-xl hover:bg-error-container transition-colors">
